...
export default function Orders() {
  ...
  const statusIcons: Record<string, string> = { confirmed: '✅', processing: '📦', shipped: '🚚', delivered: '🏠', completed: '✅', cancelled: '❌', returned: '🔄' };
  const statusLabels: Record<string, string> = { confirmed: 'Confirmed', processing: 'Processing', shipped: 'Shipped', delivered: 'Delivered', completed: 'Completed', cancelled: 'Cancelled', returned: 'Returned' };

  return (
    <div className="px-3 pt-3 pb-4 max-w-lg mx-auto">
      <h2 className="text-base font-bold mb-3">📦 {t('orders', language)} ({orders.length})</h2>
      {orders.length === 0 ? (
          ...
          return (
            <div key={o.orderNumber} className="bg-card rounded-xl border border-border p-3 mb-2 cursor-pointer hover:shadow-md transition-all" onClick={() => navigate(`/orders/${o.orderNumber}`)}>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className={cn('text-[9px] px-2 py-0.5 rounded-full font-semibold', o.status === 'confirmed' || o.status === 'completed' ? 'bg-green-100 text-green-700 dark:bg-green-950/30 dark:text-green-400' : 'bg-orange-100 text-orange-700 dark:bg-orange-950/30 dark:text-orange-400')}>
              ...
              <div className="flex gap-1 mt-2">
                {['✅', '📦', '🚚', '🏠'].map((ic, i) => (
                  <div key={i} className={cn('flex-1 h-1 rounded', i <= activeIdx ? 'bg-green-500' : 'bg-muted')} />
                ))}
              </div>
            </div>
          );
