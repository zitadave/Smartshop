...
import { Search, ShoppingCart, SlidersHorizontal, X, Heart, ChevronDown } from 'lucide-react';
...
const SORTS = [
  { id: '', label: '🔥 Best' },
  { id: 'price_low', label: '💰 Low' },
  { id: 'price_high', label: '💰 High' },
  { id: 'newest', label: '🆕 New' },
];
    ...
    <div>
      ...
      <div className="sticky top-14 z-30 bg-background/80 backdrop-blur-xl border-b border-border">
        <div className="px-4 pt-3 pb-2.5">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-2.5 rounded-2xl border border-input bg-card text-sm focus:outline-none focus:ring-2 focus:ring-ring transition-all placeholder:text-muted-foreground/60"
              value={search}
        ...
        <div className="flex items-center gap-2 px-4 pb-2.5">
          <div className="flex gap-1.5 overflow-x-auto scrollbar-none flex-1">
            {CATEGORIES.map(cat => (
              <button
                key={cat.id}
                className={cn(
                  'flex items-center gap-1 px-3 py-1.5 rounded-xl text-[10px] font-medium whitespace-nowrap border transition-all flex-shrink-0',
                  category === cat.id
          ...
          <button
            className="p-2 rounded-xl bg-card border border-border text-muted-foreground hover:bg-muted transition-colors flex-shrink-0"
        ...
        {/* Sort bar */}
        {showFilters && (
          <div className="px-4 pb-2.5 animate-slideUp">
            <div className="flex gap-1.5 overflow-x-auto scrollbar-none">
              {SORTS.map(s => (
                <button
                  key={s.id}
                  className={cn(
                    'px-3 py-1.5 rounded-xl text-[10px] font-medium border transition-all flex-shrink-0',
                    sort === s.id
        ...
        {/* Results count */}
        <div className="px-4 pb-2 text-[10px] text-muted-foreground font-medium">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-24 px-4">
          ...
          <p className="text-xs text-muted-foreground/60 mb-4">Try adjusting your search or filters</p>
          <button className="px-6 py-2.5 bg-primary text-white rounded-xl text-sm font-semibold shadow-sm hover:shadow-md transition-all" onClick={() => { setSearch(''); setCategory('all'); }}>
            Clear Filters
          </button>
        </div>
      ) : (
          ...
          {filtered.map(p => (
            <div key={p.id} className="bg-card rounded-2xl overflow-hidden border border-border cursor-pointer card-glow group"
              ...
              <div className="relative aspect-square overflow-hidden bg-muted img-zoom">
                ...
                {p.badge && (
                  <span className={cn(
                    'absolute top-2 left-2 px-2 py-0.5 rounded-md text-[8px] font-bold text-white z-10 shadow-lg',
              ...
              <div className="p-3">
                <div className="text-xs font-bold line-clamp-2 leading-snug min-h-[2em]">{p.name}</div>
                ...
                <div className="flex items-center gap-1.5 mt-1.5">
                  ...
                  {p.originalPrice && (
                    <>
                      <span className="text-[9px] text-muted-foreground line-through">{formatPrice(p.originalPrice)}</span>
                ...
                <button className="w-full mt-2.5 py-2.5 rounded-xl bg-primary text-white text-[11px] font-bold hover:bg-primary/90 active:scale-95 transition-all flex items-center justify-center gap-1.5 btn-ripple shadow-sm"
