...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (path === '/api/seed' && method === 'GET') {
      ...
      const v = await getV();
      return ok({ products: pc.count || 0, telegramUsers: uc.data?.length || 0, vendors: v.length, message: 'Smart Shop API running on Vercel!', buildId: 'BUILD-2026-08-02-V71000' });
    }
