  ...
  try {
    ...
    if (path === '/api/settings') {
      if (method === 'GET') {
        ...
        if (data?.data) return res.json({ success: true, settings: data.data });
      ...
      if (method === 'PUT') {
        ...
        if (existing) {
          await supabase.from('settings').update({ data: { ...(existing.data || existing), ...body } }).eq('id', existing.id);
        } else {
    ...
    if (path.startsWith('/api/orders')) {
      ...
      if (method === 'POST' && (path === '/api/orders' || path === '/api/')) {
        const body = { ...req.body, order_number: req.body.orderNumber || 'ETH-' + Date.now().toString(36).toUpperCase() };
        const { data } = await supabase.from('orders').insert(body).select().single();
        return res.json({ success: true, order: data || body });
      }
      if (method === 'GET') {
    ...
    if (path.startsWith('/api/vendors')) {
      ...
      if (method === 'PUT') {
        const id = parseInt(path.split('/').pop() || '0');
        await supabase.from('vendors').update(req.body).eq('id', id);
        return res.json({ success: true });
      }
    }
    ...
    if (path === '/api/analytics') {
      ...
      const topProducts = [...(products || [])].sort((a: any, b: any) => (b.sold_count || 0) - (a.sold_count || 0)).slice(0, 5)
        .map((p: any) => ({ name: p.name_en, sold: p.sold_count || 0, revenue: (p.sold_count || 0) * (p.price || 0) }));
      return res.json({ analytics: { totalProducts, totalSold, totalRevenue, totalOrders, pendingOrders, shippedOrders, topProducts } });
    }
    ...
    if (path === '/api/users/register' && method === 'POST') {
      ...
      return res.json({ success: true, user: data || req.body });
    }
    ...
    if (path === '/api/broadcast' && method === 'POST') {
      return res.json({ success: true, sent: 1, total: 1 });
    }
    ...
    if (path.startsWith('/api/pre-orders')) {
      if (method === 'GET') {
        ...
        return res.json({ preOrders: data || [] });
      }
      if (method === 'POST') {
        const { data } = await supabase.from('pre_orders').insert(req.body).select().single();
        return res.json({ success: true, preOrder: data });
      }
    }

    // ======================== CURRENCY RATES ========================
    if (path === '/api/currency/rates' && method === 'GET') {
      return res.json({ rates: { ETB: 1, USD: 0.019, EUR: 0.017, GBP: 0.015, KES: 2.45 }, base: 'ETB' });
    }

    // ======================== RECEIPTS ========================
    if (path.startsWith('/api/receipts/') && method === 'GET') {
      const on = path.replace('/api/receipts/', '');
      return res.json({ success: true, receipt: { orderNumber: on, generatedAt: new Date().toISOString() } });
    }
    ...
    if (path === '/api/flash-deals' && method === 'GET') {
      ...
      return res.json({ deals: Object.entries(flashSales).map(([pid, d]: any) => ({ id: parseInt(pid), productId: parseInt(pid), ...d })) });
    }
    ...
    if (path.startsWith('/api/tracking/')) {
      ...
      if (method === 'GET') {
        const { data } = await supabase.from('orders').select('*').eq('order_number', on).single();
        return res.json({ success: true, tracking: data?.tracking || null });
      }
      if (method === 'PUT') {
        await supabase.from('orders').update({ tracking: req.body }).eq('order_number', on);
        return res.json({ success: true });
      }
    }
    ...
    if (path === '/api/upload' && method === 'POST') {
      return res.json({ url: 'https://placehold.co/400x400/e2e8f0/94a3b8?text=Uploaded' });
    }

    // ======================== SEED / HEALTH ========================
    if (path === '/api/seed' && method === 'GET') {
      const { count } = await supabase.from('products').select('*', { count: 'exact', head: true });
      return res.json({ products: count || 0, message: 'Smart Shop API running on Vercel!', version: '3.0' });
    }
